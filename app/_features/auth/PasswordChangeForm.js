"use client";

import { useRef, useState } from 'react';
import { Eye, EyeSlash } from '@phosphor-icons/react';

export default function PasswordChangeForm({
  error,
  onChangePassword,
  embedded = false,
  onSuccess,
  titleId = 'password-change-title',
}) {
  const [currentPassword, setCurrentPassword] = useState('');
  const [password, setPassword] = useState('');
  const [confirmation, setConfirmation] = useState('');
  const [localError, setLocalError] = useState('');
  const [showPassword, setShowPassword] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const submittingRef = useRef(false);

  const handleSubmit = async (event) => {
    event.preventDefault();
    if (submittingRef.current) return;
    if (password !== confirmation) {
      setLocalError('两次输入的密码不一致');
      return;
    }
    setLocalError('');
    submittingRef.current = true;
    setSubmitting(true);
    try {
      const changed = await onChangePassword({
        currentPassword,
        newPassword: password,
      });
      if (changed) onSuccess?.();
    } finally {
      submittingRef.current = false;
      setSubmitting(false);
    }
  };

  const Root = embedded ? 'div' : 'main';

  return (
    <Root className={embedded
      ? 'w-full font-sans'
      : 'flex min-h-[100dvh] w-full items-center justify-center bg-background px-5 py-10 font-sans'}>
      <section className="w-full max-w-[400px]" aria-labelledby={titleId}>
        {!embedded && (
          <div className="mb-10 text-xl font-bold tracking-tight text-foreground">Manager.</div>
        )}
        <h1 id={titleId} className={embedded
          ? 'text-xl font-semibold tracking-tight text-foreground'
          : 'text-3xl font-semibold tracking-tight text-foreground'}>
          修改密码
        </h1>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          {embedded ? '输入并确认新的安全密码' : '首次登录需要设置新的安全密码'}
        </p>

        <form onSubmit={handleSubmit} className={`${embedded ? 'mt-6' : 'mt-8'} space-y-5`}>
          <div className="space-y-2">
            <label htmlFor="current-password" className="block text-[13px] font-medium text-foreground">
              当前密码
            </label>
            <input
              id="current-password"
              name="current-password"
              type={showPassword ? 'text' : 'password'}
              autoComplete="current-password"
              autoFocus
              required
              value={currentPassword}
              onChange={(event) => setCurrentPassword(event.target.value)}
              className="input"
            />
          </div>

          <div className="space-y-2">
            <label htmlFor="new-password" className="block text-[13px] font-medium text-foreground">
              新密码
            </label>
            <div className="relative">
              <input
                id="new-password"
                name="new-password"
                type={showPassword ? 'text' : 'password'}
                autoComplete="new-password"
                required
                value={password}
                onChange={(event) => setPassword(event.target.value)}
                className="input pr-11"
              />
              <button
                type="button"
                title={showPassword ? '隐藏密码' : '显示密码'}
                aria-label={showPassword ? '隐藏密码' : '显示密码'}
                aria-pressed={showPassword}
                onClick={() => setShowPassword((visible) => !visible)}
                className="absolute inset-y-0 right-0 flex w-11 items-center justify-center rounded-r-md text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-primary"
              >
                {showPassword ? <EyeSlash size={18} /> : <Eye size={18} />}
              </button>
            </div>
          </div>

          <div className="space-y-2">
            <label htmlFor="confirm-password" className="block text-[13px] font-medium text-foreground">
              确认新密码
            </label>
            <input
              id="confirm-password"
              name="confirm-password"
              type={showPassword ? 'text' : 'password'}
              autoComplete="new-password"
              required
              value={confirmation}
              onChange={(event) => setConfirmation(event.target.value)}
              className="input"
            />
          </div>

          <div aria-live="polite" className="min-h-5 text-[13px] leading-5 text-danger">
            {localError || error}
          </div>

          <button type="submit" disabled={submitting} className="btn btn-primary h-10 w-full">
            {submitting ? '保存中...' : '保存新密码'}
          </button>
        </form>
      </section>
    </Root>
  );
}
